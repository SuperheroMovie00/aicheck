...
public class DateChooserJButtonEx extends JButton {
    ...
    private class DateChooser extends JPanel implements ActionListener, ChangeListener {
        ...
        private JDialog createDialog(Frame owner) {
            JDialog result = new JDialog(owner, "时间选择器", true);
        ...
        void showDateChooser(Point position) {
            Frame owner = (Frame) SwingUtilities.getWindowAncestor(DateChooserJButtonEx.this);
        ...
        private void flushWeekAndDay() {     
            ...
            // 重置日历星期六、星期日的数字颜色
            for(int i = 0; i < 6; i++) {
            	if(!daysButton[i][0].getText().equals("")) {
            		daysButton[i][0].setForeground(weekendFontColor);
            	}
            	if(!daysButton[i][6].getText().equals("")) {
            ...
            for(int i = 0; i < 6; i++) {
            	for(int j = 0; j < 7; j++) {
            		if(daysButton[i][j].getText().equals(date1[2])) {
            			daysButton[i][j].setForeground(todayBackColor);
            		}
            	}
            }
