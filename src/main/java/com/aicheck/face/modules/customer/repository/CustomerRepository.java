...
*/
    ...
    @Query(value = "select DAY(ir.create_time) as day,count(ir.id) as count from visitors_record ir " +
            "where ir.customer_id is null and ir.create_time >= ?1 and ir.create_time <= ?2 group by DAY(ir.create_time) order by DAY(ir.create_time)", nativeQuery = true)
    List<Map<String, Object>> statisticalOldPassengerFlowByMonth(Date startTime, Date endTime);
    ...
    */
    @Query(value = "select MONTH(ir.create_time) as mon,count(ir.id) as count from visitors_record ir " +
            "where ir.customer_id is null and  ir.create_time >= ?1 and ir.create_time <= ?2 group by MONTH(ir.create_time) order by MONTH(ir.create_time)", nativeQuery = true)
    List<Map<String, Object>> statisticalOldPassengerFlowByMonthER(Date startTime, Date endTime);
